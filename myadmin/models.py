...
# 如果不指定表名，Dngino默认创建的数据库表为"myapp_user",Meta类将表名指定为'user'
    class Meta:
        db_table = 'user'  #更改表名
...
class Shop(models.Model):
    ...
    def toDict(self):
        ...
        return {'id': self.id, 'name': shopname[0], 'shop': shopname[1], 'cover_pic': self.cover_pic, 'banner_pic': self.banner_pic, 'address': self.address, 'phone': self.phone, 'status': self.status,
...
class Category(models.Model):
    ...
    status = models.IntegerField(default=1) # 状态：1正常/2删除
    create_at = models.DateTimeField(default=datetime.now) # 创建时间
    update_at = models.DateTimeField(default=datetime.now) # 修改时间
...
class Product(models.Model):
    ...
    def toDict(self):
        return {'id': self.id, 'shop_id': self.shop_id, 'category_id': self.category_id,
                'cover_pic': self.cover_pic, 'name': self.name, 'price': self.price,
